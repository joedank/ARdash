            ...
            >
              ...
              <template v-else>
                <span :class="column.truncate ? 'truncate block' : ''">
                  {{ getClientValue(client, column.key) }}
                </span>
              </template>
            </td>
          </tr>
...
// Removed BaseSkeletonLoader import as it doesn't exist
...
const props = defineProps({
  ...
  formatClientType: { type: Function, required: true },
  getClientTypeBadgeVariant: { type: Function, required: true },
...
// Local state for v-model binding on BaseSelect
