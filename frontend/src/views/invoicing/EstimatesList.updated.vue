        ...
        <div>
          ...
          >
            ...
            <option v-for="client in clients" :key="client.id" :value="client.id">
              {{ client.displayName }} {/* Use camelCase from clientService */}
            </option>
          </select>
        </div>
...
import estimateService from '@/services/standardized-estimates.service'
...
const loadEstimates = async () => {
  ...
  try {
    ...
    if (response.success) {
      const rawEstimates = Array.isArray(response.data) ? response.data : (response.data && Array.isArray(response.data.estimates)) ? response.data.estimates : [];
      // Convert estimates to camelCase
      estimates.value = rawEstimates.map(est => toCamelCase(est));
    } else {
      console.error('Unexpected response format or error:', response)
      handleError(new Error('Unexpected response format from estimates service'))
      estimates.value = []
    }
  } catch (err) {
...
const loadClients = async () => {
  try {
    ...
    if (response.success && response.data) {
      ...
      handleError(new Error('Failed to load clients data'))
    }
  } catch (err) {
...
const loadAssessmentProjects = async () => {
  ...
  try {
    // Using projectsService to get assessment projects
    ...
    if (response.success && response.data) {
      ...
      assessmentProjects.value = rawProjects
      ...
      handleError(new Error('Failed to load assessment projects'), 'Failed to load assessment projects')
...
const proceedToExternalLLM = async () => {
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      selectedAssessmentData.value = toCamelCase(response.data);
    } else {
      console.error('Failed to load assessment data:', response.message || 'Unknown error')
