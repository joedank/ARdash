<template>
  <BaseCard variant="bordered">
    <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Client Management</h3>
    <p class="text-gray-600 dark:text-gray-400 mb-6">Manage your clients and their addresses</p>
    
    <!-- Client Settings Component -->
    <ClientSettings />
  </BaseCard>
</template>

<script setup>
import BaseCard from '../../components/data-display/BaseCard.vue';
import ClientSettings from '../../components/settings/ClientSettings.vue';
</script>
