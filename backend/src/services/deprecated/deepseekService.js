 ...
 * Please use languageModelProvider.js instead which supports multiple providers
 * and respects settings from the database.
 */

const OpenAI = require('openai');
const logger = require('../../utils/logger'); // Adjusted path for deprecated folder
...
class DeepSeekService {
   ...
   * @param {string} model - Embedding model to use (default: 'deepseek-embedding-002')
   * @returns {Promise<Array<number>>} - Array of embedding values
   * @throws {Error} If the API call fails or client is not initialized
   */
  async generateEmbedding(text, model = 'deepseek-embedding-002') {
    if (!this.client) throw new Error('DeepSeek client not initialized');
    if (!text || text.length < 5) throw new Error('Text too short for embedding');

    try {
      logger.debug(`Generating embedding for text using model: ${model}`);
      ...
      if (!resp.data || !resp.data[0] || !resp.data[0].embedding) {
...
module.exports = {
  generateChatCompletion: deepSeekServiceInstance.generateChatCompletion.bind(deepSeekServiceInstance),
  generateEmbedding: deepSeekServiceInstance.generateEmbedding.bind(deepSeekServiceInstance),
  parseJsonResponse: deepSeekServiceInstance.parseJsonResponse.bind(deepSeekServiceInstance),
  default: deepSeekServiceInstance
};